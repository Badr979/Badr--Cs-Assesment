...
def append():
    if len(name.get()) != 0 :
        customer_details.append([name.get(),Reciept.get(),item.get(),quantity.get()])
        name.delete(0,'end')
        Reciept.delete(0,'end')
        item.delete(0,'end')
...
button_append = ttk.Button(main_window, text="Append details", command=append)
button_deleterow = ttk.Button(main_window, text="Delete row")
button_print.grid(row=7, column=1)
button_quit.grid(row=7, column=2)
button_append.grid(row=7, column=3)
button_deleterow.grid(row=7, column=4)
